...
def initialize(__loader, macros, nothing, template):
    ...
    def render(__stream, econtext, rcontext, __i18n_domain=None, __i18n_context=None):
        ...
        try:
            ...
            # <Interpolation value=<Substitution 'STR_GRF_NAME                                                    :${repo_vars.repo_title}\n\n' (1:0)> braces_required=True translation=False at 7f29f4100e48> -> __content_139818181960344
            __token = 0
            __token = 67
            __content_139818181960344 = _lookup_attr(getitem('repo_vars'), 'repo_title')
            __content_139818181960344 = __quote(__content_139818181960344, '\x00', '&#0;', None, False)
            __content_139818181960344 = ('%s%s%s' % ('STR_GRF_NAME                                                    :', (__content_139818181960344 if (__content_139818181960344 is not None) else ''), '\n\n', ))
            ...
            __append('\n')
            __backup_ship_139818164562408 = get('ship', __marker)

            # <Value 'ships' (4:38)> -> __iterator
            __token = 149
            __iterator = getitem('ships')
            (__iterator, ____index_139818165079736, ) = getitem('repeat')('ship', __iterator)
            econtext['ship'] = None
            for __item in __iterator:
                econtext['ship'] = __item

                # <Interpolation value=<Substitution '\nSTR_NAME_${ship.id}                                          :${ship.get_name_substr()}{STRING} ' (4:45)> braces_required=True translation=False at 7f29f4100a20> -> __content_139818181960344
                __token = 157
                __token = 168
                __content_139818181960344 = _lookup_attr(getitem('ship'), 'id')
                __content_139818181960344 = __quote(__content_139818181960344, '\x00', '&#0;', None, False)
                __token = 221
                __content_139818181960344_219 = _lookup_attr(getitem('ship'), 'get_name_substr')()
                __content_139818181960344_219 = __quote(__content_139818181960344_219, '\x00', '&#0;', None, False)
                __content_139818181960344 = ('%s%s%s%s%s' % ('\nSTR_NAME_', (__content_139818181960344 if (__content_139818181960344 is not None) else ''), '                                          :', (__content_139818181960344_219 if (__content_139818181960344_219 is not None) else ''), '{STRING} ', ))
                if (__content_139818181960344 is None):
                    pass
                else:
                    if (__content_139818181960344 is False):
                ...
                if (__content_139818181960344 is not None):
                    __append(__content_139818181960344)
                __append('\n')
                ____index_139818165079736 -= 1
                if (____index_139818165079736 > 0):
                    __append('')
            if (__backup_ship_139818164562408 is __marker):
                del econtext['ship']
            else:
                econtext['ship'] = __backup_ship_139818164562408
            __append('\n')
        except:
